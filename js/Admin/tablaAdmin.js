class ProductosDataTable {
  ...
  init() {
    this.dataTable = this.table.DataTable({
      ajax: {
        url: "/Mundo-Libreria/pages/Admin/obtener_productos.php?_t=" + new Date().getTime(), // Cache-busting
        type: "GET",
        dataSrc: (json) => {
  ...
  getColumns() {
    // const baseUrl = window.location.origin; // No se necesita añadir /Mundo-Libreria si el DocumentRoot es Mundo-Libreria
  ...
  static async updateDashboardMetrics() {
    try {
      const response = await fetch("/Mundo-Libreria/pages/Admin/obtener_metricas.php");
